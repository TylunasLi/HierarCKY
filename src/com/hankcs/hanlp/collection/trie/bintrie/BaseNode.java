...
public abstract class BaseNode<V> implements Comparable<BaseNode>
{
     ...
     * 遍历到一个叶子节点的处理方法
     */
    public static interface IBinTrieWalker<V>
    {
        /**
         * 遍历到一个叶子节点
         *
    ...
    protected void walk(StringBuilder prefix, IBinTrieWalker<V> walker)
    {
        prefix.append(c);
        if (status == Status.WORD_MIDDLE_2 || status == Status.WORD_END_3)
        {
            walker.onNodeWalked(prefix.toString(), value);
        }
        if (child == null) return;
        for (BaseNode node : child)
        {
            if (node == null) continue;
            node.walk(new StringBuilder(prefix.toString()), walker);
        }
    }
    ...
    public String toString()
    {
        if (child == null)
        {
            return "BaseNode{" + "c:" + c + "value:" + value +
                    "status=" + status + '}';
        }
        return "BaseNode{" +
                "child=" + child.length +
