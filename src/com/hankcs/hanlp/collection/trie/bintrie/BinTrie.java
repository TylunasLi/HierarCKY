...
import com.hankcs.hanlp.utility.GlobalParams;

import java.io.*;
import java.util.*;

import static com.hankcs.hanlp.utility.GlobalParams.logger;
...
public class BinTrie<V> extends BaseNode<V> implements ITrie<V>, Externalizable
{
    ...
    public Set<String> keySet()
    {
        ...
        {
            if (node == null) continue;
            node.walk(new StringBuilder(sb.toString()), new IBinTrieWalker<V>() {
                @Override
    ...
    public void walk(IBinTrieWalker<V> walker)
    {
        StringBuilder sb = new StringBuilder();
        for (BaseNode<V> node : child)
        {
            if (node == null) continue;
            node.walk(new StringBuilder(sb.toString()), walker);
        }
    }
    ...
    public boolean save(String path)
    {
        ...
        {
            logger.warning("保存到" + path + "失败" + GlobalParams.exceptionToString(e));
            return false;
        }
    ...
    public boolean save(DataOutputStream out)
    {
        ...
        {
            logger.warning("保存到" + out + "失败" + GlobalParams.exceptionToString(e));
            return false;
        }
